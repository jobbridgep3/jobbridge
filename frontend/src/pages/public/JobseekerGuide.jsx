...
import { JOBSEEKER_GUIDE_STEPS } from '../../config/guidesContent'
...
export default function JobseekerGuide() {
  return (
    <motion.div {...fadeIn} className="mx-auto max-w-3xl space-y-6 p-6">
      <div>
        <h1 className="text-xl font-semibold text-text-primary">Jobseeker Guide</h1>
        <p className="mt-1 text-sm text-text-muted">
          A step-by-step walkthrough of how to find and land a job through JobBridge.
        </p>
      </div>

      <div className="space-y-3">
        {JOBSEEKER_GUIDE_STEPS.map(({ step, icon: Icon, title, description }) => (
          <Card key={step}>
      ...
      <div className="flex justify-center">
        <Button asChild size="lg">
          <Link to="/register/choose">Get Started as a Jobseeker</Link>
        </Button>
      </div>
    </motion.div>
  )
