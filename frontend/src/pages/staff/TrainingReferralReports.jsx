...
export default function TrainingReferralReports() {
  ...
  const exportParams = { status: status || undefined, batch_id: batchId || undefined, date_from: dateFrom || undefined, date_to: dateTo || undefined, batch_status: batchStatus || undefined }

  const handleExport = async (format) => {
    setExporting(format)
    try {
      await downloadFile(`/api/staff/training-referral/export/${format}`, {
        params: exportParams,
        filename: `manpower_skills_report.${format === 'excel' ? 'xlsx' : 'pdf'}`,
      })
    } catch (err) {
  ...
  return (
    <motion.div {...fadeIn} className="space-y-4">
      <PageHeader
        ...
        description="Read-only view of all ManPower Skills Management applications and batches."
        actions={
          <>
