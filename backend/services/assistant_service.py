...
_SYSTEM_KNOWLEDGE = (
    ...
    "are PESO programs a Jobseeker applies to with a program-specific form and "
    "supporting documents — each has its own distinct review flow, not a shared one:\n"
    "  - SPES: an application starts pending review; if approved, the applicant proceeds "
    "to a PESO orientation (attendance tracked via QR scan), which staff records as "
    "passed or failed; a passed orientation leads to an exam (also QR-attendance "
    "tracked), which staff likewise records as passed or failed. An application can be "
    "rejected outright at the initial review stage instead.\n"
    "  - OWWA: an application starts pending; staff verifies it, then submits it to "
    "OWWA on the applicant's behalf and the case sits awaiting OWWA's response; it "
    "concludes as completed or declined. Staff can also decline it at an earlier stage.\n"
    "  - DILP: an application starts pending; staff schedules an interview; attendance "
    "is recorded as completed or no-show (a no-show can be rescheduled); a completed "
    "interview leads to an approval decision, after which the proposal becomes ready "
    "for the applicant to claim and is submitted to the Employment Services Field "
    "Office (ESFO) for funding processing.\n"
    "  Across all three, staff can request additional documents without rejecting the "
    "application outright, and exact eligibility/benefits differ per program.\n"
    ...
    "with a required reason on rejection); individual documents can separately be "
    "marked verified or need resubmission. Staff can later suspend an accredited "
    "company (blocking further activity) and reinstate it back to accredited.\n"
    "- Jobseeker profile verification: PESO staff can only mark a profile verified once "
    "it reaches 100% completion on its required fields (enforced on both the button and "
    "the API itself); marking one NOT verified always requires a written reason, shown "
    "to the jobseeker.\n"
    "- Announcements move from draft to published (optionally scheduled for a future "
    "date, and optionally set to auto-expire) to archived, and can be targeted at "
    "everyone or specific roles; only Admin can pin an announcement to the top, capped "
    "at 5 pinned at once. They can include a banner image, a gallery of images, and one "
    "PDF attachment.\n"
    "- Settings: every user can set a light/dark/system theme preference; notification "
    "and privacy preferences exist as UI-level settings. Only Admin can change "
    "system-wide configuration values.\n"
    "- Reports/exports: besides the dashboards' own Export buttons, you (Job Bot) can "
    "directly generate and deliver a downloadable report file (Excel, PDF, DOCX, or "
    "plain text, depending on what's asked) when a user explicitly asks to generate or "
    "export one — e.g. their own applications, referrals, or employment history for a "
    "Jobseeker; their own postings, applicants, or hires for an Employer; jobseeker/"
    "employer/vacancy lists and program applications for Staff/Admin; a system-wide "
    "dashboard summary or the audit trail for Admin only. This always pulls real data "
    "through a fixed, whitelisted query for that exact report — never AI-generated "
    "content — and the resulting download link expires in 15 minutes.\n"
    ...
    "- Manpower Skills Training: a Jobseeker enrolls in a listed session, which is "
    "confirmed immediately if a slot is open or waitlisted if the session is already "
    "full; PESO staff mark attendance via a QR-code scan at the session, and once "
    "attended can issue that Jobseeker a downloadable completion certificate.\n"
    "- Manpower Training Referral (a separate program from Manpower Skills Training "
    "above, run in coordination with TESDA): a Jobseeker expresses interest in a "
    "training/skill area rather than enrolling directly. PESO staff periodically pool "
    "enough interested, qualifying applicants together into a batch (a minimum of 15 "
    "people per batch), then prepare and submit a project proposal to TESDA on the "
    "batch's behalf. The Jobseeker is notified once they're pooled into a batch, and "
    "again once TESDA responds (confirmed or declined) — this program only tracks the "
    "referral pipeline up to TESDA's response, not the training itself.\n"
    ...
    "a data discrepancy.\n"
    "- What JobBridge is: a free public employment-facilitation platform run by PESO "
    "Pila, Laguna — there are no fees to register, apply, post vacancies, or use any "
    "referral service, for either Jobseekers or Employers. PESO Pila's goal is helping "
    "residents find jobs and giving employers/policymakers labor-market information; its "
    "mission covers the full employment cycle and lifelong learning for Pileños.\n"
    "- Services offered: platform features are Job Matching (skills-based vacancy "
    "matching), Referral Services, Job Fair participation, Employment Monitoring, and "
    "Career Guidance. PESO also facilitates several government programs referenced "
    "elsewhere in this knowledge (SPES, DILP, OWWA Assistance, Manpower Skills Training "
    "with TESDA) plus Special/Local Recruitment Activities (SRA/LRA) for local, "
    "overseas, and land-based employment opportunities.\n"
    "- Step-by-step visual guides exist as dedicated pages for both roles (the "
    "Jobseeker Guide and Employer Guide, linked from the public site) if a user wants a "
    "walkthrough rather than a text answer — feel free to point them there in addition "
    "to explaining in chat.\n"
    "- Terms of Use (in brief): using JobBridge means agreeing to use it only for "
    "connecting jobseekers, employers, and PESO Pila for legitimate hiring purposes; "
    "users are responsible for the accuracy of what they submit, and PESO staff may "
    "verify information or deactivate accounts with false/fraudulent details; misuse "
    "(misleading listings, unlawful discrimination, harvesting contact details) is "
    "prohibited.\n"
    "- Privacy Policy (in brief): JobBridge collects only what's needed to run the "
    "matching/referral service (registration and profile info like name, contact "
    "details, resume, employment history), shares a profile with an employer only to "
    "facilitate an application/interview, never sells personal data to third parties, "
    "and follows the Data Privacy Act of 2012 (Republic Act No. 10173). A user can "
    "request access to, correction of, or deletion of their data by contacting PESO "
    "Pila, and can deactivate their own account from Settings at any time.\n"
    "- The Citizen's Charter is PESO Pila's official document of service standards and "
    "external government programs, available as a downloadable PDF from the public "
    "site — point users there for the authoritative, complete version rather than "
    "quoting specifics you're not certain are accurate.\n"
