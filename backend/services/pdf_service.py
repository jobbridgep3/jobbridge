...
SPES_STAT_LABELS = {
    "total_applications": "Total Applications", "pending_review": "Pending Review",
    "approved_for_orientation": "Approved for Orientation", "attended_orientation": "Attended Orientation",
    "failed_orientation": "Failed Orientation", "passed": "Passed", "failed": "Failed",
    "for_deployment": "Awaiting Deployment", "deployed": "Deployed", "completed": "Completed",
    ...
    "pass_rate": "Pass Rate (%)", "dtr_compliance_rate": "DTR Compliance Rate (%)",
...
def generate_spes_report(stats: dict, rows: list[dict], date_str: str, generated_by: str, generated_role: str) -> bytes:
    """SPES report — KPI summary + application detail table, same summary/section
    shape as generate_owwa_report/generate_manpower_report, reusing the same helpers."""
    summary_rows = "".join(
    ...
    columns = [
        ...
        ("status", "Status"), ("gwa", "GWA"), ("family_income", "Family Income"), ("submitted_at", "Submitted"),
        ("reviewed_at", "Reviewed"), ("orientation_outcome_at", "Orientation Outcome"), ("exam_result_at", "Exam Result"),
    ]
    ...
    <h3>Summary</h3>
    <table>{summary_rows}</table>
    <h3>Applications ({len(rows)})</h3>
    {_series_table(rows, columns)}
    <div class="footer">JobBridge — PESO Pila, Laguna official report</div>
    """
    return _render(body, landscape=True)
