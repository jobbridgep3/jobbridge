...
_ORIENTATION_RESULT_STATUSES = ("orientation_passed", "attended_exam", "passed", "failed")


def _spes_orientation_result_label(row: dict) -> str:
    if row["status"] == "failed_orientation":
        return "Failed"
    if row["status"] in _ORIENTATION_RESULT_STATUSES:
        return "Passed"
    return "Pending"


def _spes_exam_result_label(row: dict) -> str:
...
def generate_spes_report(stats: dict, rows: list[dict], date_str: str, generated_by: str, generated_role: str, batch_name: str | None = None) -> bytes:
    """SPES report — KPI summary + separate Orientation-results and Exam-results
    tables (attendance and pass/fail are tracked independently per the module's
    QR-attendance + Staff-outcome design), same summary/section shape as
    ...
    orientation_rows = [
        {**r, "orientation_attendance": "Attended" if r["orientation_attended"] else "Absent", "orientation_result": _spes_orientation_result_label(r)}
        for r in rows
    ]
    exam_rows = [
        {**r, "exam_attendance": "Attended" if r["exam_attended"] else "Absent", "exam_result": _spes_exam_result_label(r)}
        for r in rows
    ]
    orientation_columns = [
        ("application_ref_no", "Reference #"), ("jobseeker_name", "Applicant"), ("batch_name", "Batch"),
        ("orientation_attendance", "Attendance"), ("orientation_result", "Result"),
    ]
    exam_columns = [
        ("application_ref_no", "Reference #"), ("jobseeker_name", "Applicant"), ("batch_name", "Batch"),
        ("exam_attendance", "Attendance"), ("exam_result", "Result"),
    ]

    body = f"""
    <h2>SPES Program Report</h2>
    <p>Batch: {_esc(batch_name or "All Batches")}</p>
    <p>Generated: {_esc(date_str)} | Generated By: {_esc(generated_by)} ({_esc(generated_role)})</p>
    <h3>Summary</h3>
    <table>{summary_rows}</table>
    <h3>Orientation / Interview Results ({len(orientation_rows)})</h3>
    {_series_table(orientation_rows, orientation_columns)}
    <h3>Exam Results ({len(exam_rows)})</h3>
    {_series_table(exam_rows, exam_columns)}
    <div class="footer">JobBridge — PESO Pila, Laguna official report</div>
    """
    return _render(body, landscape=True)
