...
SPES_STAT_LABELS = {
    "total_applicants": "Total Applicants", "pending_review": "Pending Review", "rejected": "Rejected",
    "approved_for_orientation": "Approved for Orientation",
    "orientation_attended": "Orientation Attended", "orientation_absent": "Orientation Absent",
    "orientation_passed": "Orientation Passed", "orientation_failed": "Orientation Failed",
    "exam_attended": "Exam Attended", "exam_absent": "Exam Absent",
    "exam_passed": "Exam Passed", "exam_failed": "Exam Failed",
    "orientation_attendance_rate": "Orientation Attendance Rate (%)", "exam_attendance_rate": "Exam Attendance Rate (%)",
    "pass_rate": "Exam Pass Rate (%)",
    "budget_allocation": "Budget Allocation (Reference)", "wage_subsidy_employer_share": "Wage Subsidy — Employer Share (60%, Reference)",
    "wage_subsidy_dole_share": "Wage Subsidy — DOLE Share (40%, Reference)",
}

def _spes_fmt_dt(value, fmt: str = "%b %d, %Y %I:%M %p") -> str:
    """spes_report_row() already returns Manila-converted datetimes (and plain date
    objects for date_of_birth) — format directly, no further conversion needed here."""
    return value.strftime(fmt) if value else ""


def generate_spes_report(
    stats: dict, rows: list[dict], date_str: str, generated_by: str, generated_role: str,
    batch_name: str | None = None, filters_summary: list[str] | None = None, columns: list[tuple] | None = None,
) -> bytes:
    """SPES report — KPI summary + one comprehensive per-applicant detail table
    (replacing the old split Orientation/Exam sections, which duplicated the same 5
    sparse columns into both — every applicant now gets one row with their full
    orientation AND exam detail side by side), same summary/section shape as
    generate_owwa_report/generate_manpower_report, reusing the same helpers.

    `columns`: (spes_report_row() dict key, header label) pairs — passed by the caller
    (blueprints/spes.py's _SPES_APPLICATION_COLUMNS) so there's one canonical column
    list shared with the Excel export, not two independently-maintained copies."""
    summary_rows = "".join(
        f"<tr><th style='width:280px'>{_esc(SPES_STAT_LABELS.get(k, k))}</th><td>{_val(v)}</td></tr>"
        for k, v in stats.items()
    )
    filters_html = "".join(f"<p>{_esc(line)}</p>" for line in (filters_summary or [f"Batch: {batch_name or 'All Batches'}"]))

    date_keys = {"submitted_at", "orientation_at", "exam_at", "peso_appointment_at"}
    formatted_rows = [
        {
            **r,
            **{k: _spes_fmt_dt(r[k]) for k in date_keys if k in r},
            "date_of_birth": _spes_fmt_dt(r.get("date_of_birth"), "%b %d, %Y"),
        }
        for r in rows
    ]
    application_columns = columns or [
        ("application_ref_no", "Reference #"), ("jobseeker_name", "Applicant"), ("batch_name", "Batch"),
        ("orientation_result_label", "Orientation Result"), ("exam_result_label", "Exam Result"), ("status_label", "Current Status"),
    ]

    body = f"""
    <h2>SPES Program Report</h2>
    <p><b>Filters:</b></p>
    {filters_html}
    <p>Generated: {_esc(date_str)} | Generated By: {_esc(generated_by)} ({_esc(generated_role)})</p>
    <h3>Summary</h3>
    <table>{summary_rows}</table>
    <h3>Applications ({len(formatted_rows)})</h3>
    {_series_table(formatted_rows, application_columns)}
    <div class="footer">JobBridge — PESO Pila, Laguna official report</div>
    """
    return _render(body, landscape=True)
...
DILP_STAT_LABELS = {
    "total_applications": "Total Applications", "pending": "Pending", "scheduled": "Scheduled",
    "completed": "Completed", "no_show": "No-Show", "ready_for_claiming": "Ready for Claiming",
    "approved": "Approved", "submitted_to_esfo": "Submitted to ESFO",
    "avg_pending_to_esfo_days": "Average Pending → ESFO Time (days)", "no_show_rate": "No-Show Rate (%)",
}


def _dilp_fmt_dt(value, fmt: str = "%b %d, %Y %I:%M %p") -> str:
    """dilp_report_row() already returns Manila-converted datetimes — format directly,
    no further conversion needed here."""
    return value.strftime(fmt) if value else ""


def generate_dilp_report(stats: dict, rows: list[dict], date_str: str, generated_by: str, generated_role: str) -> bytes:
    """DILP report — KPI summary + application detail table, same summary/section shape as
    generate_owwa_report/generate_manpower_report, reusing the same helpers. Rendered
    landscape since the Applications table has 12 columns."""
    summary_rows = "".join(
        f"<tr><th style='width:280px'>{_esc(DILP_STAT_LABELS.get(k, k))}</th><td>{_val(v)}</td></tr>"
        for k, v in stats.items()
    )
    ...
    formatted_rows = [
        {
            **r,
            "created_at": _dilp_fmt_dt(r["created_at"]),
            "interview_at": _dilp_fmt_dt(r["interview_at"]),
            "completed_at": _dilp_fmt_dt(r["completed_at"]),
            "ready_for_claiming_at": _dilp_fmt_dt(r["ready_for_claiming_at"]),
            "approved_at": _dilp_fmt_dt(r["approved_at"]),
            "submitted_to_esfo_at": _dilp_fmt_dt(r["submitted_to_esfo_at"]),
            "capital_needed": f"₱{r['capital_needed']:,.2f}" if r.get("capital_needed") is not None else "",
        }
        for r in rows
    ]
    columns = [
        ("reference_number", "Reference #"), ("jobseeker_name", "Applicant"), ("proposed_livelihood", "Proposed Livelihood"),
        ("capital_needed", "Capital Needed"), ("status", "Status"), ("created_at", "Date Submitted"),
        ("interview_at", "Interview Date"), ("completed_at", "Completed"), ("ready_for_claiming_at", "Ready for Claiming"),
        ("approved_at", "Approved"), ("submitted_to_esfo_at", "Submitted to ESFO"), ("no_show_count", "No-Show Count"),
    ]
