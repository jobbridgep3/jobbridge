...
def _query_spes_applications(args):
    """Delegates to spes_reporting_service's shared filter builder — the single source
    of truth for SpesApplication filtering (status/batch/date-range/search), so this
    endpoint's results can never diverge from Reports' stats/export filtering for the
    same filter values (see the module-level docstring on _spes_application_filters)."""
    query = spes_reporting_service._spes_application_filters(
        SpesApplication.query.options(*_APP_EAGER),
        batch_id=args.get("batch_id"), status=args.get("status"),
        date_from=args.get("date_from"), date_to=args.get("date_to"), search=args.get("search"),
    )
