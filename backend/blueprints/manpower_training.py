...
from services import manpower_capacity_service, manpower_reporting_service
...
from services.pdf_service import generate_manpower_report, to_bytesio
...
# ---------- Reporting (staff + admin, additive/read-only — no existing route touched) ----------

@manpower_training_bp.get("/staff/training-referral/stats")
@jwt_required()
@role_required("staff", "admin")
def staff_manpower_stats():
    """Global (unfiltered) KPI aggregate for the dashboard cards — always reflects the
    whole dataset regardless of whatever filters are selected on the Reports page."""
    return ok(manpower_reporting_service.build_manpower_stats())
...
def _parse_report_filters(args):
    ...
    return {
        ...
        "date_from": date_from,
        "date_to": date_to,
        "batch_status": args.get("batch_status") or None,
    }


@manpower_training_bp.get("/staff/training-referral/export/excel")
@jwt_required()
@role_required("staff", "admin")
def export_manpower_excel():
    from services.pdf_service import MANPOWER_STAT_LABELS

    filters = _parse_report_filters(request.args)
    applications = manpower_reporting_service.query_applications_for_report(
        status=filters["status"], batch_id=filters["batch_id"], date_from=filters["date_from"], date_to=filters["date_to"],
    )
    batches = manpower_reporting_service.query_batches_for_report(filters["batch_status"])
    stats = manpower_reporting_service.build_manpower_stats()
    ...
    summary_rows = [
        ...
        ["", ""],
        ["Metric", "Value"],
        *[[MANPOWER_STAT_LABELS.get(k, k), v] for k, v in stats.items()],
    ]
    application_rows = [
        [d["jobseeker_name"], d["email"], d["program_interest"], d["batch_name"], d["status"], d["application_date"], d["remarks"]]
        for d in (manpower_reporting_service.application_report_row(a) for a in applications)
    ]
    batch_rows = [
        [d["batch_name"], d["status"], d["min_pax"], d["max_pax"], d["current_pax"], d["slots_remaining"], d["submitted_to_tesda_date"], d["tesda_response_date"]]
        for d in (manpower_reporting_service.batch_report_row(b) for b in batches)
    ]

    buf = build_multi_sheet_excel_report([
        ("Summary", ["PESO Pila, Laguna — ManPower Skills Management Report", ""], summary_rows),
        ("Applications", ["Applicant", "Email", "Program Interest", "Batch", "Status", "Application Date", "Remarks"], application_rows),
        ("Batches", ["Batch Name", "Status", "Min Pax", "Max Pax", "Current Pax", "Slots Remaining", "Submitted to TESDA", "TESDA Response"], batch_rows),
    ])
    log_audit(user, "Export", "training_referral_report", details="excel")
    return send_file(
        buf, mimetype="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        as_attachment=True, download_name="manpower_skills_report.xlsx",
    )


@manpower_training_bp.get("/staff/training-referral/export/pdf")
@jwt_required()
@role_required("staff", "admin")
def export_manpower_pdf():
    filters = _parse_report_filters(request.args)
    applications = manpower_reporting_service.query_applications_for_report(
        status=filters["status"], batch_id=filters["batch_id"], date_from=filters["date_from"], date_to=filters["date_to"],
    )
    ...
    application_rows = [manpower_reporting_service.application_report_row(a) for a in applications]
    batch_rows = [manpower_reporting_service.batch_report_row(b) for b in batches]

    pdf_bytes = generate_manpower_report(stats, application_rows, batch_rows, date_str, user.email, user.role)
    log_audit(user, "Export", "training_referral_report", details="pdf")
    return send_file(
        to_bytesio(pdf_bytes), mimetype="application/pdf",
        as_attachment=True, download_name="manpower_skills_report.pdf",
    )
